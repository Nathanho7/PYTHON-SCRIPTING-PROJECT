...
class App:
    ...
    def __init__(self, root):
        ...
        self.root.geometry("900x600") # Légèrement plus large pour la nouvelle colonne
    ...
    def _build_header(self):
        ...
        header.pack(fill="x")
        tk.Label(header, text="⚽ Gestionnaire de Performances", font=("Helvetica", 20, "bold"), bg="#313244", fg="#cdd6f4").pack()
    ...
    def _build_footer(self):
        self.label_total = tk.Label(self.root, text="", font=("Helvetica", 13, "bold"), bg="#1e1e2e", fg="#f9e2af")
        self.label_total.pack(pady=5)

    def ajouter_footballeur(self):
        try:
            nom = self.entry_nom.get()
            buts, passes, matchs = int(self.entry_buts.get()), int(self.entry_passes.get()), int(self.entry_matchs.get())
    ...
    def rafraichir_tableau(self):
        ...
        for f in self.manager.footballeurs:
            self.tableau.insert("", "end", values=(
                ...
                f.total_individuel() # <--- Affiche le score par joueur
            ))
        
        # Mise à jour du bandeau du bas
        self.label_total.config(text=f"Total Global : {self.manager.total_buts()} buts | {self.manager.total_passes()} passes | {self.manager.total_ga()} G/A")

    def afficher_graphique(self):
        totaux = self.manager.buts_par_poste()
        if not totaux: return
        fen = tk.Toplevel(self.root)
        fig, ax = plt.subplots(figsize=(5, 4), facecolor="#1e1e2e")
        ax.pie(totaux.values(), labels=totaux.keys(), autopct="%1.1f%%", textprops={'color':"w"})
